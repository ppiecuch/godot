...
static const uint32 relativeTagBottomLeft = (uint32)((1 << yShift) + (uint32(-1) << xShift));
...
void b2ParticleSystem::RemoveSpuriousBodyContacts()
{
	...
	int32 discarded = 0;
	auto end = std::remove_if(m_bodyContactBuffer.Begin(), m_bodyContactBuffer.End(), b2ParticleBodyContactRemovePredicate(this, &discarded));
	if (end != m_bodyContactBuffer.End())
