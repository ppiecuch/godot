...
void
aa2map_block_mirror (const st_aa2map_t *aa2map, const st_aa2map_parse_t *a)
{
...
void
aa2map_block_mirror_shader (const st_aa2map_t *aa2map, const st_aa2map_parse_t *a)
{
  (void) a;
  char name[MAXBUFSIZE];
  char s[MAXBUFSIZE];

  sprintf (name, "textures/%s/block_mirror", aa2map->path);
  sprintf (s, "{\n"  
              "surfaceparm nolightmap\n"
              "surfaceparm solid\n"
              "cull twosided\n"
              "{\n"
              "map textures/%s/block_mirror.tga\n"
              "tcGen environment\n"
