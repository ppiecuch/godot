...
void b2WheelJoint::InitVelocityConstraints(const b2SolverData& data)
{
	...
	if (m_frequencyHz > 0.0f)
	{
		...
		if (invMass > 0.0f)
		{
			...
			// Frequency
			const float32 omega = 2.0f * b2_pi * m_frequencyHz;

			// Damping coefficient
			const float32 damp = 2.0f * m_springMass * m_dampingRatio * omega;

			// Spring stiffness
			const float32 spring = m_springMass * omega * omega;

			// magic formulas
			float32 h = data.step.dt;
			m_gamma = h * (damp + h * spring);
			if (m_gamma > 0.0f)
			{
				m_gamma = 1.0f / m_gamma;
			}

			m_bias = C * h * spring * m_gamma;
