...
const AuthenticateResponse *AuthenticateResponse::get() const { return message ? this : nullptr ; }

String AuthenticateResponse::get_collation_id() const { return message->collation_id()->c_str(); }
bool AuthenticateResponse::is_valid() const { return message != nullptr; }
bool AuthenticateResponse::is_error() const { return message->id()->error() != nullptr; }
NkErrorCode AuthenticateResponse::get_error_code() const { return message->id()->error()->code(); }
String AuthenticateResponse::get_error_message() const  { return message->id()->error()->message()->c_str(); }
bool AuthenticateResponse::is_session() const { return message->id()->session() != nullptr; }
String AuthenticateResponse::get_session_token() const { return message->id()->session()->token()->c_str(); }

AuthenticateResponse::AuthenticateResponse(PoolByteArray p_payload)
: payload(p_payload)
, message(flatbuffers::GetRoot<server::AuthenticateResponse>(payload.read().ptr())) {
}

/// NkMessage

const NkMessage *NkMessage::get() const { return message ? this : nullptr; }
...
NkMessage::PayloadCase NkMessage::get_payload_case() const {
	...
	if (const server::Envelope_::EnvelopeContent *payload = get_envelope_payload()) {
		if (payload->heartbeat()) return PAYLOAD_HEARTBEAT;
		if (payload->topic_message()) return PAYLOAD_TOPIC_MESSAGE;
		if (payload->topic_presence()) return PAYLOAD_TOPIC_PRESENCE;
		if (payload->match_data()) return PAYLOAD_MATCH_DATA;
		if (payload->match_presence()) return PAYLOAD_MATCH_PRESENCE;
		if (payload->matchmake_matched()) return PAYLOAD_MATCHMAKE_MATCHED;
		if (payload->live_notifications()) return PAYLOAD_LIVE_NOTIFICATIONS;
		if (payload->error()) return PAYLOAD_ERROR;
		if (payload->self()) return PAYLOAD_SELF;
		if (payload->users()) return PAYLOAD_USERS;
		if (payload->friends()) return PAYLOAD_FRIENDS;
		if (payload->groups()) return PAYLOAD_GROUPS;
		if (payload->groups_self()) return PAYLOAD_GROUPS_SELF;
		if (payload->group_users()) return PAYLOAD_GROUP_USERS;
		if (payload->storage_data()) return PAYLOAD_STORAGE_DATA;
		if (payload->storage_keys()) return PAYLOAD_STORAGE_KEYS;
		if (payload->rpc()) return PAYLOAD_RPC;
		if (payload->topics()) return PAYLOAD_TOPICS;
		if (payload->topic_message_ack()) return PAYLOAD_TOPIC_MESSAGE_ACK;
		if (payload->topic_messages()) return PAYLOAD_TOPIC_MESSAGES;
		if (payload->match()) return PAYLOAD_MATCH;
		if (payload->matches()) return PAYLOAD_MATCHES;
		if (payload->matchmake_ticket()) return PAYLOAD_MATCHMAKE_TICKET;
		if (payload->leaderboards()) return PAYLOAD_LEADERBOARDS;
		if (payload->leaderboard_records()) return PAYLOAD_LEADERBOARD_RECORDS;
		if (payload->notifications()) return PAYLOAD_NOTIFICATIONS;
	}
...
: payload(p_payload)
, message(flatbuffers::GetRoot<server::Envelope>(payload.read().ptr())) {
