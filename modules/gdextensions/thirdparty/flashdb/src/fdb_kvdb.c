...
#define KV_SECTOR_HDR_DATA_SIZE                     (FDB_WG_ALIGN(sizeof(struct kv_sector_hdr_data)))
#define KV_SECTOR_DIRTY_OFFSET                      ((unsigned long)(&((struct kv_sector_hdr_data *)0)->status_table.dirty))
...
typedef struct kv_sector_hdr_data *kv_sector_hdr_data_t;
...
static uint32_t get_next_kv_addr(fdb_kvdb_t db, kv_sec_info_t sector, fdb_kv_t pre_kv)
{
    ...
    if (pre_kv->addr.start == FAILED_ADDR) {
        ...
        addr = sector->addr + KV_SECTOR_HDR_DATA_SIZE;
    } else {
        if (pre_kv->addr.start <= sector->addr + db_sec_size(db)) {
            ...
            addr = find_next_kv_addr(db, addr, sector->addr + db_sec_size(db) - KV_SECTOR_HDR_DATA_SIZE);
...
static fdb_err_t read_kv(fdb_kvdb_t db, fdb_kv_t kv)
{
    ...
    if (kv->len == ~0 || kv->len > db_max_size(db) || kv->len < KV_NAME_LEN_OFFSET) {
        ...
        kv->crc_is_ok = false;
        return FDB_READ_ERR;
    } else if (kv->len > db_sec_size(db) - KV_SECTOR_HDR_DATA_SIZE && kv->len < db_max_size(db)) {
...
static fdb_err_t kv_read_sector_info(fdb_kvdb_t db, uint32_t addr, kv_sec_info_t sector, bool traversal)
{
    fdb_err_t result = FDB_NO_ERR;
    struct kv_sector_hdr_data sec_hdr = { 0 };

    FDB_ASSERT(addr % db_sec_size(db) == 0);
    FDB_ASSERT(sector);

    /* read sector header raw data */
    _fdb_flash_read((fdb_db_t)db, addr, (uint32_t *)&sec_hdr, sizeof(struct kv_sector_hdr_data));
    ...
    if (traversal) {
        ...
        if (sector->status.store == FDB_SECTOR_STORE_EMPTY) {
            sector->remain = db_sec_size(db) - KV_SECTOR_HDR_DATA_SIZE;
...
static uint32_t kv_get_next_sector_addr(fdb_kvdb_t db, kv_sec_info_t pre_sec)
{
    uint32_t next_addr;
        ...
        bool (*callback)(fdb_kv_t kv, void *arg1, void *arg2))
{
    ...
    /* search all sectors */
    do {
        if (kv_read_sector_info(db, sec_addr, &sector, false) != FDB_NO_ERR) {
            continue;
        }
        if (callback == NULL) {
            continue;
    ...
    } while ((sec_addr = kv_get_next_sector_addr(db, &sector)) != FAILED_ADDR);
}
...
static fdb_err_t kv_format_sector(fdb_kvdb_t db, uint32_t addr, uint32_t combined_value)
{
    fdb_err_t result = FDB_NO_ERR;
    struct kv_sector_hdr_data sec_hdr = { 0 };
    ...
    if (result == FDB_NO_ERR) {
        ...
        memset(&sec_hdr, 0xFF, sizeof(struct kv_sector_hdr_data));
        ...
        /* save the header */
        result = _fdb_flash_write((fdb_db_t)db, addr, (uint32_t *)&sec_hdr, sizeof(struct kv_sector_hdr_data), true);
...
static fdb_err_t kv_update_sec_status(fdb_kvdb_t db, kv_sec_info_t sector, size_t new_kv_len, bool *is_full)
{
...
static void kv_sector_iterator(fdb_kvdb_t db, kv_sec_info_t sector, fdb_sector_store_status_t status, void *arg1, void *arg2,
        bool (*callback)(kv_sec_info_t sector, void *arg1, void *arg2), bool traversal_kv)
{
    ...
    sec_addr = 0;
    do {
        kv_read_sector_info(db, sec_addr, sector, false);
        if (status == FDB_SECTOR_STORE_UNUSED || status == sector->status.store) {
            if (traversal_kv) {
                kv_read_sector_info(db, sec_addr, sector, true);
            }
            /* iterator is interrupted when callback return true */
...
static uint32_t alloc_kv(fdb_kvdb_t db, kv_sec_info_t sector, size_t kv_size)
{
    ...
    kv_sector_iterator(db, sector, FDB_SECTOR_STORE_UNUSED, &empty_sector, &using_sector, sector_statistics_cb, false);
    if (using_sector > 0) {
        /* alloc the KV from the using status sector first */
        kv_sector_iterator(db, sector, FDB_SECTOR_STORE_USING, &arg, NULL, alloc_kv_cb, true);
    }
    if (empty_sector > 0 && empty_kv == FAILED_ADDR) {
        if (empty_sector > FDB_GC_EMPTY_SEC_THRESHOLD || db->gc_request) {
            kv_sector_iterator(db, sector, FDB_SECTOR_STORE_EMPTY, &arg, NULL, alloc_kv_cb, true);
        } else {
...
static fdb_err_t del_kv(fdb_kvdb_t db, const char *key, fdb_kv_t old_kv, bool complete_del)
{
    ...
    dirty_status_addr = FDB_ALIGN_DOWN(old_kv->addr.start, db_sec_size(db)) + KV_SECTOR_DIRTY_OFFSET;
...
static fdb_err_t move_kv(fdb_kvdb_t db, fdb_kv_t kv)
{
    ...
    /* start move the KV */
    {
        ...
        kv_update_sec_status(db, &sector, kv->len, NULL);
...
static bool do_gc(kv_sec_info_t sector, void *arg1, void *arg2)
{
    ...
    if (sector->check_ok && (sector->status.dirty == FDB_SECTOR_DIRTY_TRUE || sector->status.dirty == FDB_SECTOR_DIRTY_GC)) {
        ...
        _fdb_write_status((fdb_db_t)db, sector->addr + KV_SECTOR_DIRTY_OFFSET, status_table, FDB_SECTOR_DIRTY_STATUS_NUM, FDB_SECTOR_DIRTY_GC, true);
        /* search all KV */
        kv.addr.start = sector->addr + KV_SECTOR_HDR_DATA_SIZE;
        do {
            read_kv(db, &kv);
        ...
        kv_format_sector(db, sector->addr, SECTOR_NOT_COMBINED);
...
static void gc_collect(fdb_kvdb_t db)
{
    ...
    kv_sector_iterator(db, &sector, FDB_SECTOR_STORE_EMPTY, &empty_sec, NULL, gc_check_cb, false);
    ...
    if (empty_sec <= FDB_GC_EMPTY_SEC_THRESHOLD) {
        kv_sector_iterator(db, &sector, FDB_SECTOR_STORE_UNUSED, db, NULL, do_gc, false);
    }
...
static fdb_err_t create_kv_blob(fdb_kvdb_t db, kv_sec_info_t sector, const char *key, const void *value, size_t len)
{
    ...
    if (kv_hdr.len > db_sec_size(db) - KV_SECTOR_HDR_DATA_SIZE) {
    ...
    if (kv_addr != FAILED_ADDR || (kv_addr = new_kv(db, sector, kv_hdr.len)) != FAILED_ADDR) {
        ...
        if (result == FDB_NO_ERR) {
            result = kv_update_sec_status(db, sector, kv_hdr.len, &is_full);
        }
        if (result == FDB_NO_ERR) {
            uint8_t ff = 0xFF;
...
fdb_err_t fdb_kv_set_default(fdb_kvdb_t db)
{
    ...
    for (addr = 0; addr < db_max_size(db); addr += db_sec_size(db)) {
        result = kv_format_sector(db, addr, SECTOR_NOT_COMBINED);
        if (result != FDB_NO_ERR) {
            goto __exit;
        }
    }
...
void fdb_kv_print(fdb_kvdb_t db)
{
    ...
    FDB_PRINT("size: %lu/%u bytes.\n", (uint32_t)using_size + ((SECTOR_NUM - FDB_GC_EMPTY_SEC_THRESHOLD) * KV_SECTOR_HDR_DATA_SIZE),
...
static bool kv_check_sec_hdr_cb(kv_sec_info_t sector, void *arg1, void *arg2)
{
    if (!sector->check_ok) {
...
fdb_err_t _fdb_kv_load(fdb_kvdb_t db)
{
    ...
    kv_sector_iterator(db, &sector, FDB_SECTOR_STORE_UNUSED, &check_failed_count, db, kv_check_sec_hdr_cb, false);
    ...
    kv_sector_iterator(db, &sector, FDB_SECTOR_STORE_UNUSED, db, NULL, check_and_recovery_gc_cb, false);
...
bool fdb_kv_iterate(fdb_kvdb_t db, fdb_kv_iterator_t itr)
{
    ...
    do {
        if (kv_read_sector_info(db, itr->sector_addr, &sector, false) == FDB_NO_ERR) {
            if (sector.status.store == FDB_SECTOR_STORE_USING || sector.status.store == FDB_SECTOR_STORE_FULL) {
                if (kv->addr.start == 0) {
                    kv->addr.start = sector.addr + KV_SECTOR_HDR_DATA_SIZE;
                }
        ...
        */
        kv->addr.start = 0;
    } while ((itr->sector_addr = kv_get_next_sector_addr(db, &sector)) != FAILED_ADDR);
