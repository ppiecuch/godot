...
#define _T(X, Y) itos(X) + "_" + itos(Y)
	...
	DDLSGraph build_graph(Vector<Point2> p_shape) {
		DDLSGraph graph; graph.instance();
		DDLSGraphNode node;
		for (int i = 0; i < p_shape.size(); i++) {
			node = graph->insert_node();
			NodeData node_data; node_data.instance();
			node_data->index = i;
		...
		while (node1) {
			...
			while (node2 != node1) {
				...
				EdgeData edge_data; edge_data.instance();
...
namespace DDLSRectMeshFactory {
	...
	DDLSMesh build_rectangle(real_t p_width, real_t p_height) {
		DDLSVertex v_tl; v_tl.instance();
		DDLSVertex v_tr; v_tr.instance();
		DDLSVertex v_br; v_br.instance();
		DDLSVertex v_bl; v_bl.instance();

		DDLSEdge e_tl_tr; e_tl_tr.instance();
		DDLSEdge e_tr_tl; e_tr_tl.instance();
		DDLSEdge e_tr_br; e_tr_br.instance();
		DDLSEdge e_br_tr; e_br_tr.instance();
		DDLSEdge e_br_bl; e_br_bl.instance();
		DDLSEdge e_bl_br; e_bl_br.instance();
		DDLSEdge e_bl_tl; e_bl_tl.instance();
		DDLSEdge e_tl_bl; e_tl_bl.instance();
		DDLSEdge e_tr_bl; e_tr_bl.instance();
		DDLSEdge e_bl_tr; e_bl_tr.instance();
		DDLSEdge e_tl_br; e_tl_br.instance();
		DDLSEdge e_br_tl; e_br_tl.instance();

		DDLSFace f_tl_bl_tr; f_tl_bl_tr.instance();
		DDLSFace f_tr_bl_br; f_tr_bl_br.instance();
		DDLSFace f_tl_br_bl; f_tl_br_bl.instance();
		DDLSFace f_tl_tr_br; f_tl_tr_br.instance();

		DDLSConstraintShape bound_shape; bound_shape.instance();
		DDLSConstraintSegment seg_top; seg_top.instance();
		DDLSConstraintSegment seg_right; seg_right.instance();
		DDLSConstraintSegment seg_bot; seg_bot.instance();
		DDLSConstraintSegment seg_left; seg_left.instance();

		DDLSMesh mesh; mesh.instance();
