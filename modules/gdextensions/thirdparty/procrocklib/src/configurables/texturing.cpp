...
void NoiseGradientAlbedoGenerator::addOwnGroups(Configuration& config, std::string newGroupName, std::function<bool()> activeFunc) {
  noiseGraph.addOwnGroups(config, newGroupName, activeFunc);
  coloring.addOwnGroups(config, newGroupName, activeFunc);
}

void NoiseGradientAlbedoGenerator::modify(TextureGroup& textureGroup) {
  textureGroup.albedoData.resize(textureGroup.albedoChannels * textureGroup.width * textureGroup.height);
...
void AlbedoGenerator::addOwnGroups(Configuration& config, std::string newGroupName, std::function<bool()> activeFunc) {
  Configuration::ConfigurationGroup albedoGroup;
  albedoGroup.entry = {"General", "General Settings for albedo generation.", activeFunc};
  albedoGroup.singleChoices.emplace_back(Configuration::SingleChoiceEntry{
      {"Method", "Choose how to generate the albedo."},
...
void GradientNormalsGenerator::modify(TextureGroup& textureGroup) {
  ...
  switch (sourceChannel) {
    case 0:
      image = CImg<float>(textureGroup.displacementData.data(), 1, textureGroup.width, textureGroup.height);
      break;
    case 1:
      image = CImg<unsigned char>(textureGroup.albedoData.data(), textureGroup.albedoChannels, textureGroup.width, textureGroup.height);
      break;
    default:
...
void NormalsGenerator::addOwnGroups(Configuration& config, std::string newGroupName, std::function<bool()> activeFunc) {
...
// Roughness
void GreyscaleRoughnessGenerator::addOwnGroups(Configuration& config, std::string newGroupName, std::function<bool()> activeFunc) {
...
// Metallness
void GreyscaleMetalnessGenerator::addOwnGroups(Configuration& config, std::string newGroupName, std::function<bool()> activeFunc) {
  Configuration::ConfigurationGroup greyscaleGroup;
...
void MetalnessGenerator::addOwnGroups(Configuration& config, std::string newGroupName, std::function<bool()> activeFunc) {
...
// Ambient Occ.
void GreyscaleAmbientOcclusionGenerator::addOwnGroups(Configuration& config, std::string newGroupName, std::function<bool()> activeFunc) {
  Configuration::ConfigurationGroup greyscaleGroup;
...
void AmbientOcclusionGenerator::addOwnGroups(Configuration& config, std::string newGroupName, std::function<bool()> activeFunc) {
