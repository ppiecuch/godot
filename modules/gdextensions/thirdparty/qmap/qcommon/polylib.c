...
winding_t *BaseWindingForPlane (vec3_t normal, vec_t dist)
{
	...
	winding_t *w = AllocWinding (4); // project a really big axis aligned box onto the plane
...
void ClipWindingEpsilon (winding_t *in, vec3_t normal, vec_t dist, vec_t epsilon, winding_t **front, winding_t **back)
{
	...
	*front = *back = NULL;
	...
	int maxpts = in->numpoints+4; // cant use counts[0]+2 because of fp grouping errors
	...
	*back = b = AllocWinding (maxpts);
	...
	for (int i=0; i<in->numpoints; i++)
	{
		...
		vec3_t mid;
		for (int j=0 ; j<3 ; j++) // avoid round off error when possible
		{
			if (normal[j] == 1)
				mid[j] = dist;
			else if (normal[j] == -1)
