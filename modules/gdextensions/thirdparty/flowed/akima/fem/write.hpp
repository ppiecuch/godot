...
#include "format.hpp"
#include "star.hpp"
#include "str_arr_ref.hpp"
#include "utils/double_to_string.hpp"
#include "utils/misc.hpp"
#include "utils/real_as_string.hpp"
    ...
    public:
        ...
        integer_star_1 const& val)
      {
        if (io_mode == io_unformatted) {
          ...
          char buf[64];
          int n = std::snprintf(buf, 64, "%4d", static_cast<int>(val));
        ...
        integer_star_2 const& val)
      {
        if (io_mode == io_unformatted) {
          ...
          char buf[64];
          int n = std::snprintf(buf, 64, "%6d", static_cast<int>(val));
          to_stream_star(buf, n);
          prev_was_string = false;
        }
        else {
        ...
        integer_star_4 const& val)
      {
        if (io_mode == io_unformatted) {
          ...
          char buf[64];
          int n = std::snprintf(buf, 64, "%11d", val);
          to_stream_star(buf, n);
          prev_was_string = false;
        }
        else {
          ...
          if (ed[0] == 'i') {
            ...
            fmt[n+1] = '\0';
            char buf[64];
            n = std::snprintf(buf, 64, fmt, val);
            to_stream_fmt(buf, n);
          }
          else {
            char buf[64];
            int n = std::snprintf(buf, 64, " %d", val);
            to_stream_fmt(buf, n);
          }
        }
        return *this;
      ...
      protected: // implementation detail
          ...
          double const& val)
        {
          ...
          if (ed[0] == 'f') {
            ...
            char buf[64];
            int n = std::snprintf(buf, 64, " %.6g", val);
            to_stream_fmt(buf, n);
          }
        }
        public:
