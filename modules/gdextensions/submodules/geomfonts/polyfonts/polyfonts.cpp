...
const pffont *getDefaultFont() { return (&pfPSans16); }
...
void PolyFont::setScale(float s) { pfScaleX = pfScaleY = s; }

void PolyFont::setScaleXY(float sx, float sy) { pfScaleX = sx; pfScaleY = sy; }

void PolyFont::setPosition(float x, float y) { pfTextX = x; pfTextY = y; }

pfpoint PolyFont::getPosition() const { return (pfpoint){pfTextX, pfTextY}; }

void PolyFont::setSkew(float s) { pfTextSkew = MIN(1.0, MAX(-1.0, s)); }

void PolyFont::setWeight(int w) { pfTextWeight = MIN(numWeights, MAX(1, w)); }

void PolyFont::setAngleR(float a) { pfTextSin = Math::sin(a); pfTextCos = Math::cos(a); }

void PolyFont::setAngleD(float a) { setAngleR(a * DtoR); }

void PolyFont::setCenter(bool onOff) { pfCenter = onOff; }
...
const pffont *PolyFont::getCurrentFont() { return currentFont; }
...
float PolyFont::getStringWidth(float s, const char *c) { return s * getStringWidth(c); }

float PolyFont::getStringWidth(float s, const wchar_t *c) { return s * getStringWidth(c); }
...
int PolyFont::drawWideChar(wchar_t c) {
	...
	for (k = 0; k < pfTextWeight; k++) {
		...
		for (i = 0; i < segs; i++) {
			...
			coord_vector vertices; for (j = 0; j < points; j++) {
				gx = unfix(*d++);
				gy = unfix(*d++);
...
int PolyFont::drawChar(char c) { return drawWideChar(c); }
...
int PolyFont::drawString(float x, float y, float sx, float sy, const char *c) {
	setPosition(x, y); setScaleXY(sx, sy);
	return drawString(c);
}
