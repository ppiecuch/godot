        ...
        if elem['callee']['type'] == 'Identifier':
            ...
            # JSON.parse(JSON.stringify(DATA_LH)).concat(getLatin3(-52, 9))
                ...
                if 'callee' in elem['arguments'][0]:
                    return "concatPaths(%s, %s(%s, %s))" % (elem['callee']['object']['arguments'][0]['arguments'][0]['name'], elem['arguments'][0]['callee']['name'], dump_value(elem['arguments'][0]['arguments'][0]), dump_value(elem['arguments'][0]['arguments'][1]))
                else:
