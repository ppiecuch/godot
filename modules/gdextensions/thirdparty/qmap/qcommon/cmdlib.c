...
void SetQdirFromPath (char *path)
{
	...
	int len = strlen(BASEDIRNAME);
	for (char *c=path+strlen(path)-1 ; c != path ; c--)
	{
...
typedef struct 
{ 
	char filename[0x38]; // Name of the file, Unix style, with extension,  56 chars, padded with '\0'.
	...
	for (int n = 0;; n++)
	{
		...
		if (pak_entry != NULL)
		{
			if (!fseek(f, pak_header.diroffset, SEEK_SET))
			{
				...
				{
					for (int n = 0; pak_entry[i].filename[n] != 0; n++)
					{
