...
char *
realpath2 (const char *path, char *full_path) // enhanced realpath() which returns the absolute path of a file
{
...
fcopy (const char *source, size_t start, size_t len, const char *dest, const char *mode)
{
  ...
  if (!strchr ("aw", *mode))
    {
      AA2MAP_LOG_ERROR ( "ERROR: fcopy() (logically) supports only write or append as mode\n\n");
      exit (1);
    }
...
makepath (const char *path, int mode)
{
  ...
  if (strlen (path) >= FILENAME_MAX)   
    {
      AA2MAP_LOG_ERROR ( "ERROR: directory name too long\n");
      return -1;
    }
