...
void getMacOSDefaults(GLMContext glm_ctx)
{
    ...
    errorCode = CGLChoosePixelFormat(attributes, &pix, &num); ERR_FAIL_COND(errorCode == kCGLNoError);
    // add error checking here
    errorCode = CGLCreateContext(pix, NULL, &ctx); ERR_FAIL_COND(errorCode == kCGLNoError);
    CGLDestroyPixelFormat(pix);
    errorCode = CGLSetCurrentContext(ctx); ERR_FAIL_COND(errorCode == kCGLNoError);
