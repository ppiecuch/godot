...
void F2X_Context::DerivedState::update_current_color()
{
	current_color = color_to_ushort(ctx->gl_state.current.color);
	current_alpha = std::max(ctx->gl_state.current.color[3].intValue >> 11, 31);
}
...
bool F2X_Context::DerivedState::update_state_before_draw()
{
	...
	if (!ctx->select_optimized_shaders()) {
