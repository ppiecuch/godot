...
def build_legacycg_header(filename, include, class_suffix, output_attribs):
	...
	cg_vert_src = runexe(["platform/psvita/video/tools/shader_compiler", "glsl2hlsl"], input="\n".join(header_data.vertex_lines))
	cg_frag_src = runexe(["platform/psvita/video/tools/shader_compiler", "glsl2hlsl"], input="\n".join(header_data.fragment_lines))
...
def build_cg_headers(target, source, env):
	for x in source:
		build_legacycg_header(
			str(x), include="drivers/gles2/shader_gles2.h", class_suffix="GLES2", output_attribs=True
		)
