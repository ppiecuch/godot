...
# include "ldrawlib/colors.inl"
#endif

typedef std::map<unsigned int, const color_entity *> color_map_type;
typedef std::vector<unsigned int> color_index_type;

const color_map_type color::color_map = color_map_type();
color_index_type color::color_index = color_index_type();
...
const color_entity color::color_chart[] = {
  ...
  {      material_normal, {172, 120, 186, 255}, { 51,  51,  51, 255},  0,  30,                 "Medium Lavender", 0L },
  {      material_normal, {225, 213, 237, 255}, { 51,  51,  51, 255},  0,  31,                        "Lavender", 0L },
  ...
  {      material_normal, {204, 112, 42,  255}, { 51,  51,  51, 255},  0,  84,               "Medium Dark Flesh", 0L },
  ...
  {      material_normal, {20,  152, 215, 255}, { 51,  51,  51, 255},  0, 321,                      "Dark_Azure", 0L },
  {      material_normal, {62,  194, 221, 255}, { 51,  51,  51, 255},  0, 322,                    "Medium Azure", 0L },
  ...
  {      material_normal, {173,  97,  64, 255}, { 51,  51,  51, 255},  0, 128,                     "Dark Nougat", 0L },
  ...
  {    material_metallic, {26,  40,   49, 255}, { 51,  51,  51, 255},  0,  83,                  "Metallic Black", 0L },
  {    material_metallic, {109, 110,  92, 255}, { 51,  51,  51, 255},  0,  87,              "Metallic Dark Grey", 0L },
...
void color::link()
{
  ...
  if (pos == color_index.end())
    color_index.push_back( m_id );

  m_custom_color = false;
  m_valid = true;

  auto it = color_map.find(m_id);
  if (it != color_map.end()) {
    m_entity = (*it).second;
    return;
  }
  ...
  if (m_id >= 256 && m_id <= 512) {
    ...
    if ((m_id & 0xff000000) == 0x04000000) {
        ...
        nc->material = material_normal;
        nc->id = m_id;
        nc->name = "MLCad custom opaque dither color";
        ...
        if ((m_id & 0xff000000) == 0x02000000) {
            m_custom_color = true;
            unsigned int v = m_id & 0xff000000;

            color_entity *nc = new color_entity;
            nc->material = material_normal;
            nc->id = m_id;
            nc->name = "MLCad custom opaque RGB color";
            nc->rgba[0] = (unsigned char) ((v & 0xff0000) >> 16);
            nc->rgba[1] = (unsigned char) ((v & 0xff00) >> 8);
            nc->rgba[2] = (unsigned char) (v & 0xff);
            nc->rgba[3] = 255;
...
unsigned int color::from_index(unsigned int inx)
{
#ifdef LDR_ARCHIVE_SUPPORT
  if (inx < _lcolors_cnt)
    return _lcolors[inx];
  std::cerr << "[libLDR] couldn't resolve index color " << inx << std::endl;
#endif
  return 7; // default color
}

unsigned int color::get_index() const
{
  int index = 0; for(auto &e : color_index) {
    if (e == m_id)
        return index;
    ++index;
  }
  std::cerr << "[libLDR] couldn't resolve color id " << m_id << std::endl;
  return 0; // default index
}

void color::save_index()
{
    // save colors table:
    std::ofstream fcol("colors.inl", std::ios::out);
    static const int _lcolors_cnt = 155;
    fcol << "static const int _lcolors_cnt = " << color_index.size() << ";" << std::endl;
    fcol << "static unsigned int _lcolors[] = {" << std::endl;
    int cnt = 0;for(auto &c : color_index) {
        fcol << " /* " << cnt << " */ " << c;
        if (++cnt == color_index.size())
            fcol << std::endl;
        else
            fcol << "," << std::endl;
    }
