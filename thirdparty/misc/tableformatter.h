...
inline void assert(bool statement, const std::string &message) {
	if (!statement) {
...
_FORCE_INLINE_ CellFormatter::CellFormatter(size_t _width) :
		width(_width) {
}

_FORCE_INLINE_ StringVector CellFormatter::formatCellContent(const StringVector &unformattedCellChunks,
		size_t maximumNumberOfRows) {
...
_FORCE_INLINE_ TableFormatter::TableFormatter(size_t _numberOfColumns,
		size_t _columnWidth) :
...
_FORCE_INLINE_ TableFormatter::TableFormatter(const CellFormatterVector &CellFormatters) :
		numberOfColumns(CellFormatters.size()),
		cellFormatters(CellFormatters),
		columnDelimiters(CellFormatters.size() + 1, "|") {
}

_FORCE_INLINE_ void TableFormatter::setCellFormatter(const CellFormatter &cellFormatter,
		size_t column) {
	cellFormatters.at(column) = cellFormatter;
}

_FORCE_INLINE_ void TableFormatter::setColumnDelimiter(const std::string &delimiter,
		size_t columnBoundary) {
	columnDelimiters.at(columnBoundary) = delimiter;
}

_FORCE_INLINE_ void TableFormatter::digestRowStream() {
...
_FORCE_INLINE_ void TableFormatter::addRow(const StringVector &row) {
	// Digest rowStream first
...
_FORCE_INLINE_ StringVector TableFormatter::createChunks(std::string cellString,
		size_t columnWidth) {
	// Remove multiple whitespace
	auto bothAreSpaces = [](char lhs, char rhs) { return (lhs == rhs) && (lhs == ' '); };
...
_FORCE_INLINE_ std::string TableFormatter::toString() {
	this->digestRowStream();
...
_FORCE_INLINE_ size_t TableFormatter::getTableWidth() const {
	size_t width = 0;
...
_FORCE_INLINE_ void TableFormatter::clearData() {
	data.clear();
}

_FORCE_INLINE_ void TableFormatter::addHorizontalLine(const char &character) {
