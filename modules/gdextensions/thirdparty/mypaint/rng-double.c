 ...
 *
 * This version also adopts Brendan McKay's suggestion to accommodate naive
 * users who forget to call ranf_start(seed).
 *
 *
...
void rng_double_set_seed(RngDouble *self, long seed) {
	...
	for (; j < KK; j++)
		self->ran_u[j - LL] = u[j];
	for (j = 0; j < 10; j++)
		rng_double_get_array(self, u, KK + KK - 1); /* warm things up */
