...
#if HAVE_LIBPTHREAD || defined __APPLE__ || defined __QNX__
#include <pthread.h>
...
#define NoSymbol 0
KeySym XStringToKeysym(const char *sym) {
	return NoSymbol;
}
#endif
...
/* hexkey[8] MODE : Sets the internal key register according to
 * the hexadecimal key contained in the 8 bytes of hexkey,
 * according to the DES, for encryption or decryption according
 * to MODE.
 */
...
/* cookedkey[32] : Copies the contents of the internal key register
 * into the storage located at &cookedkey[0].
 */

extern void des(unsigned char *, unsigned char *);
/* from[8] to[8] : Encrypts/Decrypts (according to the key currently
 * loaded in the internal key register) one block of eight bytes at
 * address 'from' into the block at address 'to'.  They can be the same.
 */
...
#define DBG_VNC 0 // 1      1
#define DBG_HTTP 1 // 2     3
...
public:
	...
	static int socketConnect(int sock, const struct sockaddr_in *sa);
...
class VNCSockets {
private:
	char ServerName[MAXHOSTNAMELEN]; ///< name of the server
	uint16_t port; ///< port
	uint32_t ipaddr; ///< IP address of the server
	int rfbsock; ///< the used socket
	uint32_t buffered; ///< buffer
	uint8_t buf[VNC_BUF_SIZE];
	uint8_t *bufoutptr;
	bool StringToIPAddr(); ///< stores the IP address of the server in ipaddr, returns false if unknown

public:
	VNCSockets() {}; ///< constructors
	...
	bool SameMachine(); ///< Test if the other end of a socket is on the same machine.
	bool SetNonBlocking(); ///< sets a socket into non-blocking mode.
	bool ReadFromRFBServer(char *out, uint32_t n); ///< Read bytes from the sever and stores it in the buffer
	bool WriteExact(char *buf, int n); ///< Write an exact number of bytes, and don't return until you've sent them.
	int GetSock(); ///< get the socket used
};
...
typedef struct {
	...
	uint8_t pad;
	uint16_t nRects; /* followed by nRects rectangles */
...
class VNCRFBproto {
private:
	char passwordFile[64]; ///< Name of the file containing the encrypted password
...
public:
	...
	VNCRFBproto(char *ServeurName, int Port, char *pswdFile); ///< constructors

	rfbPixelFormat pixFormat;
	rfbServerInitMsg si;

	void printPixelFormat(rfbPixelFormat *format); ///< Prints out the format for debugging
	void setVisual32(); ///< default is 32 BPP
	bool connectToRFBServer(); ///< connects to the server
	bool initialiseRFBConnection(); ///< initialise the connection with this server
	...
	bool setFormatAndEncodings(); ///< sets the format of rfb messages and the way it will be encoded.

	//Send a request to the server to update the FrameBuffer
	bool sendIncrementalFramebufferUpdateRequest(); ///< if incremental, updates the Framebuffer for all the screen
	bool sendFramebufferUpdateRequest(int x, int y, int w, int h, bool incremental); ///< if not, updates only one rectangle

	int getSock(); ///< get the used socket
};
...
class VNCRGB {
public:
	VNCRGB(); ///< this constructor is only used to handle faster 32bits pixels we get from the server
...
private:
	...
	bool viewonly; ///< do we want to forward events? (no if true)

	void fillRect(int rx, int ry, int rw, int rh, VNCRGB pixel); ///< handling Rectangles we got from server
...
public:
	VNCClient(char *server, int port, char *passFile); ///< constructor

	char *serverCutText;
	bool newServerCutText; ///< for now, public attributes we keep to handle server cut texts

	VNCRGB *framebuffer; ///< here is our framebuffer
	uint16_t fbWidth;
	uint16_t fbHeight;

	int getSock(); ///< we might want to get the socket descriptor to listen to it
	...
	bool VNCInit(); ///< VNCInit has to allocate memory for the framebuffer if everything goes fine
	bool VNCClose(); ///< VNCClose has to free it

	/* Remote Frame Buffer Protocol v3.3 */

	void sendRFBEvent(char **params, unsigned int *num_params); ///< messages from client to server
	bool sendIncrementalFramebufferUpdateRequest();
	bool sendFramebufferUpdateRequest(int x, int y, int w, int h, bool incremental);

	bool handleRFBServerMessage(); ///< handle the messages from the server
};
...
public:
	...
	uint16_t realScreenWidth; ///< here we keep what the real server screen size is
...
const char *write_dir() {
	...
	static std::string _dr = GetPathFromCFURLRef(CopyDocumentsURL());
	return _dr.c_str();
#else
	return ".";
#endif
...
void trace(int dbgmask, const char *s, ...) {
	...
	if ((1 << dbgmask) || dbgmask == DBG_FORCE) {
		va_start(ap, s);
		vfprintf(stderr, s, ap);
		va_end(ap);
		fprintf(stderr, "\n");
...
FILE *writelog(const char *s, ...) {
	...
	if (fl == NULL) {
		sprintf(logfile, "%s/log", write_dir());
		unlink(logfile);
...
#if MACOSX || defined __APPLE__ || defined __QNX__
...
struct hostent *my_getipnodebyname(const char *hostname, int af) {
#if HAVE_GETIPNODEBYNAME || defined __APPLE__ || defined __QNX__
	int err;
...
#if HAVE_GETHOSTBYNAME_R || defined __APPLE__ || defined __QNX__
...
#if HAVE_GETIPNODEBYADDR || defined __APPLE__ || defined __QNX__
	int err;
...
void my_free_hostent(struct hostent *hp) {
#if HAVE_FREEHOSTENT || defined __APPLE__ || defined __QNX__
	if (hp)
		freehostent(hp);
#else
	FREE(hp);
...
/** inet_ntop, inet_ntoa */
const char *my_inet_ntop(int af, const void *addr) {
#if HAVE_INET_NTOP || defined __APPLE__ || defined __QNX__
...
/** inet_pton, inet_addr */
int my_inet_pton(int af, const char *name, void *addr) {
#if HAVE_INET_PTON || defined __APPLE__ || defined __QNX__
...
const char *inet4_ntop(const void *addr) { return my_inet_ntop(AF_INET, addr); }

int inet4_pton(const char *name, void *addr) { return my_inet_pton(AF_INET, name, addr); }

const char *inet6_ntop(const void *addr) { return my_inet_ntop(AF_INET6, addr); }

int inet6_pton(const char *name, void *addr) { return my_inet_pton(AF_INET6, name, addr); }
...
bool VNCRFBproto::sendClientCutText(char *str, int len) {
	...
	return (VNC_sock.WriteExact((char *)&cct, sz_rfbClientCutTextMsg) && VNC_sock.WriteExact(str, len));
}
