...
typedef struct
{
	...
	int capacity; // Number of allocated points.
	int16_t *points; // Point array.
} contour_t;
...
typedef struct
{
	label_t label; // Label assigned to this blob.
	contour_t external; // External contour.
	contour_t *internal; // Internal contours (holes).
	int internal_count; // Number of internal contour (also called euler number).
} blob_t;
...
static int contour_trace(uint8_t external, label_t current, int16_t x, int16_t y,
	...
	while (!done) {
		...
		int previous = (i + 4) & 7; /* 1. Compute the neighbour index of the previous point. */
		i = (previous + 2) & 7; /* 2. Next search index is previous + 2 (mod 8). */
	}
...
int find_blobs(int16_t roi_x, int16_t roi_y, int16_t roi_w, int16_t roi_h,
	...
	/* sanity check. */
	if ((nullptr == label) || (nullptr == label_w) || (nullptr == label_h) || (nullptr == blobs) || (nullptr == count)) {
	...
	*label_h = roi_h;
	...
	for (int j = 0; j < roi_h; j++, line_in += in_w) {
		for (int i = 0, ptr_in = line_in; i < roi_w; i++, ptr_in++, ptr_label++) {
			if (0 == *ptr_in) {
				continue;
			}
