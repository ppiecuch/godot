...
and many more (see ```submodules``` folder) - they are groupped in subdirectories and can be individually enabled/disabled using ```gdext_enable_submodules``` cli parameters (see: ```config.py```). Some code might be private (and encrypted) - just remove them or exclude from building.
