...
void
aa2map_block_water (const st_aa2map_t *aa2map, const st_aa2map_parse_t *a)
{
...
void
aa2map_block_water_shader (const st_aa2map_t *aa2map, const st_aa2map_parse_t *a)
{
  (void) a;
  char name[MAXBUFSIZE];
  char s[MAXBUFSIZE];

  sprintf (name, "textures/%s/block_water", aa2map->path);
  sprintf (s, "{\n"
              ...
              "cull twosided\n"
              "{\n"
              "map textures/%s/block_water.tga\n"
              "tcGen environment\n"
