...
#define USE_HASHING // undefine to make plane finding use linear sort
...
int CreateNewFloatPlane (vec3_t normal, vec_t dist)
{
	...
	if ((!badnormal_check && len < 0.5) || (badnormal_check && (len + badnormal < 1 || len - badnormal < -1)))
	{
		if(show_points)
...
int FindFloatPlane (vec3_t normal, vec_t dist)
{
	...
	for (int i=0, p=mapplanes ; i<nummapplanes ; i++, p++)
	{
...
int BrushContents (mapbrush_t *b)
{
	...
	int contents = s->contents;
	int trans = texinfo[s->texinfo].flags;
...
void ParseBrush (entity_t *mapent)
{
	...
	if (b->contents & CONTENTS_ORIGIN)
	{
		...
		if (num_entities == 1)
		{
			Error ("Entity %i, Brush %i: origin brushes not allowed in world\n  BrushL %s", b->entitynum, b->brushnum, brush_info);
			return;
		}
		...
		b->numsides = 0; // don't keep this brush
...
void MoveBrushesToWorld (entity_t *mapent)
{
	...
	int worldbrushes = entities[0].numbrushes;

	mapbrush_t *temp = malloc(newbrushes*sizeof(mapbrush_t));
...
qboolean ParseMapEntity (void)
{
	...
	if (!strcmp ("func_areaportal", ValueForKey (mapent, "classname")))
	{
		...
		SetKeyValue (mapent, "style", str); // set the portal number as "style"
		MoveBrushesToWorld (mapent);
		return true;
	}
...
void TestExpandBrushes (void)
{
	...
	for (int bn=0; bn<nummapbrushes ; bn++)
	{
		mapbrush_t *brush = &mapbrushes[bn];
		fprintf (f, "{\n");
		for (int i=0; i<brush->numsides ; i++)
		{
			side_t *s = brush->original_sides + i;
			vec_t dist = mapplanes[s->planenum].dist;
			for (int j=0 ; j<3 ; j++)
				dist += fabs( 16 * mapplanes[s->planenum].normal[j] );

			winding_t *w = BaseWindingForPlane (mapplanes[s->planenum].normal, dist);
