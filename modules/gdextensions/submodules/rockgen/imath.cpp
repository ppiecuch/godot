...
/*****************************************************/
/*                                                   */
/* Calcule le plus grand facteur commun entre a et b */
...
/*                                             */
/*   Le compact vient du fait que cette        */
/*   routine prend 5 instructions seulement... */
/*                                             */
/*   xor eax,eax                               */
/*   mov ecx,i                                 */
/*  @ici:                                      */
...
unsigned int FloorSqrt(unsigned int x)
 { 
  unsigned int y=0;
  unsigned int base = 32768;
  x++;
  while (base)
   {
    unsigned int t = y+base;
    if ( (t*t) < x )  y=t;
    base >>=1;
   }
  return y;
...
/* le code est bien écrit, je le refais moi- */
/* même.                                     */
/*********************************************/
...
/* Les nombres premiers sont encodés en différentiel dans
   ...
   2) L'algorithme de factorisation parcourre la table séquentiellement
      de toutes façons!
...
/***************************************************/
/*                                                 */
/* Détermine si deux nombres sont relativement     */
/* premiers                                        */
/*                                                 */
/***************************************************/
...
/*                                                  */
/****************************************************/
unsigned int RelativementPremierSqrt(unsigned int a)
 {
  unsigned int t = Sqrt(a); // sqrt(a);
  unsigned int i=0;
  int c1,c2;
  do 
   {
...
/*                                                    */
/******************************************************/
unsigned int RelativementPremierPhi(unsigned int a)
 {
  unsigned int t = (int)(InverseGoldenRatio * a + 0.5); // sqrt(a);
  unsigned int i=0;
  int c1,c2;
  do 
   {
...
void FacteursPropres(unsigned int i, unsigned int **facteursPropres, int & factptr)
 {
  ...
  for (j=0;j<l;j++) regs[j]=0;

  (*facteursPropres) = (unsigned int *)malloc(factptr * sizeof(unsigned int));

  for (k=0;k<factptr;k++)
   {
    unsigned int s=1;

    for (j=0;j<l;j++) s*=puiss[j][regs[j]];
...
unsigned int GenerateurStochastique(unsigned int q)
 {
  unsigned int facteurs[32];
  unsigned int m[32]; // 10-12, en pire cas
  int f,i,j,k,ok;
  unsigned int l;
  ...
  for (i=0;i<f;i++) m[i]=(q-1)/facteurs[i];
      ...
      while ((l==0) && (k<1000))
       {
        k++;
        while ((j=(rand() % q)) <2);
        for (l=0,i=0;i<f;i++) l+= (ModExpoDiscrete(j, m[i], q ) == 1);
       }
...
unsigned int PlusPetitGenerateur(unsigned int q)
 {
  ...
  do {
      ...
      while ((l==0) && (k<q))
       {
        k++;
        for (l=0,i=0;i<f;i++) l+= (ModExpoDiscrete(k, m[i], q ) == 1);
       }
...
unsigned int PlusPetiteNiemeRacineUnite(unsigned int q, unsigned int m)
 {
  if ((q-1) % m)
       return 0xffffffffu;
  else return ModExpoDiscrete(PlusPetitGenerateur(q), (q-1)/m, q);
 }
