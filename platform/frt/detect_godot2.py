...
def configure_glsl_builders(env):
    ...
    env.Append(
        BUILDERS={
            "GLSL120": env.Builder(action=methods.build_legacygl_headers, suffix="glsl.gen.h", src_suffix=".glsl")
        }
    )
    env.Append(
        BUILDERS={"GLSL": env.Builder(action=methods.build_glsl_headers, suffix="glsl.gen.h", src_suffix=".glsl")}
    )
    env.Append(
        BUILDERS={
            "GLSL120GLES": env.Builder(action=methods.build_gles2_headers, suffix="glsl.gen.h", src_suffix=".glsl")
        }
    )
