...
static inline bool _fuzzy_compare(double p1, double p2) { return (Math::abs(p1 - p2) * 1000000000000. <= MIN(Math::abs(p1), Math::abs(p2))); }
static inline bool _fuzzy_compare(float p1, float p2) { return (Math::abs(p1 - p2) * 100000.f <= MIN(Math::abs(p1), Math::abs(p2))); }
static inline bool _fuzzy_is_null(double d) { return Math::abs(d) <= 0.000000000001; }
static inline  bool _fuzzy_is_null(float f) { return Math::abs(f) <= 0.00001f; }
...
inline Vector2 operator*(const Vector2 &p, const Matrix2D &m) { return m.map(p); }

inline bool Matrix2D::is_identity() const
{
    return _fuzzy_is_null(_m11 - 1) && _fuzzy_is_null(_m22 - 1) && _fuzzy_is_null(_m12)
           && _fuzzy_is_null(_m21) && _fuzzy_is_null(_dx) && _fuzzy_is_null(_dy);
}

inline bool _fuzzy_compare(const Matrix2D& m1, const Matrix2D& m2)
{
    return _fuzzy_compare(m1.m11(), m2.m11())
        && _fuzzy_compare(m1.m12(), m2.m12())
        && _fuzzy_compare(m1.m21(), m2.m21())
        && _fuzzy_compare(m1.m22(), m2.m22())
        && _fuzzy_compare(m1.dx(), m2.dx())
        && _fuzzy_compare(m1.dy(), m2.dy());
}
