...
EXPORT bool SetRasterizeSettings(ToveRasterizeSettings *settings, const char *algorithm, TovePaletteRef palette, float spread, float noise, const float *noiseMatrix, int noiseMatrixSize);
...
EXPORT int SubpathCurveTo(ToveSubpathRef subpath, float cpx1, float cpy1, float cpx2, float cpy2, float x, float y);
EXPORT int SubpathArc(ToveSubpathRef subpath, float x, float y, float r, float startAngle, float endAngle, bool counterclockwise);
EXPORT int SubpathDrawRect(ToveSubpathRef subpath, float x, float y, float w, float h, float rx, float ry);
...
EXPORT void SubpathSet(ToveSubpathRef subpath, ToveSubpathRef source, float a, float b, float c, float d, float e, float f);
...
EXPORT void PathRemoveSubpath(TovePathRef path, ToveSubpathRef traj);
...
EXPORT void PathSet(TovePathRef path, TovePathRef source, bool scaleLineWidth, float a, float b, float c, float d, float e, float f);
...
EXPORT void GraphicsRemovePath(ToveGraphicsRef shape, TovePathRef path);
...
EXPORT void GraphicsSet(ToveGraphicsRef graphics, ToveGraphicsRef source, bool scaleLineWidth, float a, float b, float c, float d, float e, float f);
EXPORT void GraphicsRasterize(ToveGraphicsRef shape, uint8_t *pixels, int width, int height, int stride, float tx, float ty, float scale, const ToveRasterizeSettings *settings);
...
EXPORT void MeshSetVertexBuffer(ToveMeshRef mesh, void *buffer, int32_t size);
EXPORT ToveTrianglesMode MeshGetIndexMode(ToveMeshRef mesh);
EXPORT int MeshGetIndexCount(ToveMeshRef mesh);
EXPORT void MeshCopyIndexData(ToveMeshRef mesh, void *buffer, int32_t size);
...
EXPORT ToveShaderCode GetGPUXFillShaderCode(const ToveShaderData *data, bool fragLine, bool meshBand, bool debug);
...
EXPORT ToveMeshUpdateFlags TesselatorTessGraphics(ToveTesselatorRef tess, ToveGraphicsRef graphics, ToveMeshRef mesh, ToveMeshUpdateFlags flags);
EXPORT ToveMeshUpdateFlags TesselatorTessPath(ToveTesselatorRef tess, ToveGraphicsRef graphics, TovePathRef path, ToveMeshRef fillMesh, ToveMeshRef lineMesh, ToveMeshUpdateFlags flags);
