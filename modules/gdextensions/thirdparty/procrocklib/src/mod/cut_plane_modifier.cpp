#include <procrocklib/mod/cut_plane_modifier.h>
...
CutPlaneModifier::CutPlaneModifier() {
  ...
  group.float3s.emplace_back(Configuration::BoundedEntry<Eigen::Vector3f>{{"Translation", "Translate the plane in space."}, &translation, {-2, -2, -2}, {2, 2, 2}});
  ...
  group.bools.emplace_back(Configuration::SimpleEntry<bool>{{"Noisy Surface", "Use a noise graph for the surface of the plane."}, &useNoise});

  Configuration::ConfigurationGroup noiseGroup;
  noiseGroup.entry = {"Noise Settings", "Settings for the applied noise", showNoiseFunc};

  noiseGroup.noiseGraphs.emplace_back(Configuration::SimpleEntry<NoiseGraph>{{"Noise Graph", "The graph to define the applied noise."}, &noiseGraph});
  noiseGroup.ints.emplace_back(Configuration::BoundedEntry<int>{{"Resolution", "Resolution of mesh to have before applying noise."}, &resolution, 5, 9});
  noiseGroup.floats.emplace_back(Configuration::BoundedEntry<float>{{"Scale", "Scaling of noise depth in plane."}, &scale, 0.001f, 0.2f});
