...
# error "Embedded archive support is required for this module to build."
    ...
    check_deps = [&](const std::vector<ldcatalog::handle_t> &hndls, std::vector<ldcatalog::handle_t> &lst) {
      for(auto &h : hndls)
        if (std::find(lst.begin(), lst.end(), h) == lst.end()) {
          lst.push_back(h); check_deps(m_reader->get_archive_deps(h), lst);
        }
    };
    auto opt = [=](ldraw::model *m, std::vector<ldcatalog::handle_t> &lst) {
      for (int i = 0; i < m->size(); ++i) {
        if (m->at(i)->get_type() == ldraw::type_ref) {
          ...
          const std::string fn = ldraw::utils::translate_string(r->filename()); if (m_reader->get_archive_exists(fn)) {
            const ldcatalog::handle_t h=m_reader->get_archive_handle(fn);
            if (std::find(lst.begin(), lst.end(), h) == lst.end()) {
              lst.push_back(h); check_deps(m_reader->get_archive_deps(fn), lst);
            }
          }
        }
      }
    ...
    for(auto &sm : m->submodel_list()) opt(sm.second, load_list);
    ...
    if (int(xp.count(geometry_exporter::buffer_type::type_lines)>0)
        + int(xp.count(geometry_exporter::buffer_type::type_triangles)>0)
        + int(xp.count(geometry_exporter::buffer_type::type_quads)>0) == 0) // lines-triangles-quads
    ...
    if (xp.count(geometry_exporter::buffer_type::type_lines) > 0) {
        ...
        std::map<unsigned int, int> unique_colors; for (int c=0; c<numFaces; ++c)
            unique_colors[colors[c]]++;
        
        // build mesh for each material/color:
        int cnt = 1; for(auto &col : unique_colors) {
            ...
            unsigned int index = 0; for (unsigned int t = 0; t < numFaces; ++t) {
...
#endif
    int mindex = 0; for(auto &m : allMaterials)
    {
