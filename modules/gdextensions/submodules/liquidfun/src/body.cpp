...
static WorldB2 *make_body_test_world() {
	return memnew(WorldB2(memnew(b2World(b2Vec2(0.0f, -9.8f)))));
}

TEST_SUITE("[liquidfun] BodyDefB2") {
	TEST_CASE("[liquidfun] BodyDefB2 defaults match b2BodyDef") {
		BodyDefB2 *def = memnew(BodyDefB2);
		CHECK(def->get_position() == Vector2(0.0f, 0.0f));
		CHECK(def->get_angle() == doctest::Approx(0.0f));
		CHECK(def->get_body_type() == b2_staticBody);
		CHECK(def->get_gravity_scale() == doctest::Approx(1.0f));
		CHECK(def->get_allow_sleep() == true);
		CHECK(def->get_awake() == true);
		CHECK(def->get_fixed_rotation() == false);
		CHECK(def->get_bullet() == false);
		CHECK(def->get_active() == true);
		CHECK(def->get_linear_damping() == doctest::Approx(0.0f));
		CHECK(def->get_angular_damping() == doctest::Approx(0.0f));
		memdelete(def);
	}

	TEST_CASE("[liquidfun] BodyDefB2 setters roundtrip") {
		BodyDefB2 *def = memnew(BodyDefB2);
		def->set_position(Vector2(3.0f, 5.0f));
		def->set_angle(1.2f);
		def->set_body_type(b2_dynamicBody);
		def->set_gravity_scale(0.5f);
		def->set_bullet(true);
		def->set_fixed_rotation(true);
		def->set_linear_damping(0.1f);
		def->set_angular_damping(0.2f);
		CHECK(def->get_position() == Vector2(3.0f, 5.0f));
		CHECK(def->get_angle() == doctest::Approx(1.2f));
		CHECK(def->get_body_type() == b2_dynamicBody);
		CHECK(def->get_gravity_scale() == doctest::Approx(0.5f));
		CHECK(def->get_bullet() == true);
		CHECK(def->get_fixed_rotation() == true);
		CHECK(def->get_linear_damping() == doctest::Approx(0.1f));
		CHECK(def->get_angular_damping() == doctest::Approx(0.2f));
		memdelete(def);
	}
} // TEST_SUITE("[liquidfun] BodyDefB2")

TEST_SUITE("[liquidfun] BodyB2") {
	TEST_CASE("[liquidfun] BodyB2 position and type match def") {
		WorldB2 *w = make_body_test_world();
		BodyDefB2 *def = memnew(BodyDefB2);
		def->set_position(Vector2(2.0f, -3.0f));
		def->set_body_type(b2_dynamicBody);
		BodyB2 *body = def->instance(w);
		memdelete(def);
		CHECK(body->get_position().x == doctest::Approx(2.0f));
		CHECK(body->get_position().y == doctest::Approx(-3.0f));
		CHECK(body->get_body_type() == b2_dynamicBody);
		memdelete(w);
	}

	TEST_CASE("[liquidfun] BodyB2 set_position and set_angle") {
		WorldB2 *w = make_body_test_world();
		BodyDefB2 *def = memnew(BodyDefB2);
		BodyB2 *body = def->instance(w);
		memdelete(def);
		body->set_position(Vector2(1.5f, 2.5f));
		body->set_angle(0.5f);
		CHECK(body->get_position().x == doctest::Approx(1.5f));
		CHECK(body->get_position().y == doctest::Approx(2.5f));
		CHECK(body->get_angle() == doctest::Approx(0.5f));
		memdelete(w);
	}

	TEST_CASE("[liquidfun] BodyB2 no fixtures or joints initially") {
		WorldB2 *w = make_body_test_world();
		BodyDefB2 *def = memnew(BodyDefB2);
		BodyB2 *body = def->instance(w);
		memdelete(def);
		CHECK(body->get_fixture_list().size() == 0);
		CHECK(body->get_joint_list().size() == 0);
		memdelete(w);
	}

	TEST_CASE("[liquidfun] BodyB2 metadata transferred from def") {
		WorldB2 *w = make_body_test_world();
		BodyDefB2 *def = memnew(BodyDefB2);
		def->set_metadata(42);
		BodyB2 *body = def->instance(w);
		memdelete(def);
		CHECK(int(body->get_metadata()) == 42);
		memdelete(w);
	}

	TEST_CASE("[liquidfun] BodyB2 damping and gravity_scale get/set") {
		WorldB2 *w = make_body_test_world();
		BodyDefB2 *def = memnew(BodyDefB2);
		def->set_body_type(b2_dynamicBody);
		BodyB2 *body = def->instance(w);
		memdelete(def);
		body->set_linear_damping(0.3f);
		body->set_angular_damping(0.1f);
		body->set_gravity_scale(2.0f);
		CHECK(body->get_linear_damping() == doctest::Approx(0.3f));
		CHECK(body->get_angular_damping() == doctest::Approx(0.1f));
		CHECK(body->get_gravity_scale() == doctest::Approx(2.0f));
		memdelete(w);
	}

	TEST_CASE("[liquidfun] BodyB2 get_world returns correct world") {
		WorldB2 *w = make_body_test_world();
		BodyDefB2 *def = memnew(BodyDefB2);
		BodyB2 *body = def->instance(w);
		memdelete(def);
		CHECK(body->get_world() == w);
		memdelete(w);
	}
} // TEST_SUITE("[liquidfun] BodyB2")
