...
void LMSurfaceGatherer::surface_gatherer_reset_state() {
	for (int s = 0; s < out_surfaces.surface_count; ++s) {
		...
		if (surf->vertices != nullptr) {
			free(surf->vertices);
			surf->vertices = nullptr;
		}

		if (surf->indices != nullptr) {
			free(surf->indices);
			surf->indices = nullptr;
		}
	}

	if (out_surfaces.surfaces != nullptr) {
		free(out_surfaces.surfaces);
		out_surfaces.surfaces = nullptr;
	}
