...
std::vector<std::string> split_string(const std::string &s, char delim)
{
    ...
    std::string item; while(getline(ss, item, delim)) {
...
std::string &replaceall(std::string &input, std::string fromstr, std:: string tostr) {
	size_t pos = 0; while(pos != std::string::npos) {
		pos = input.find(fromstr);
		if(pos == std::string::npos) continue;
		input.erase(pos, fromstr.length());
		input.insert(pos, tostr, tostr.length());
	}
	return input;
}
...
{
	const float v = m.value(0, 0)*m.value(1, 1)*m.value(2, 2)
		+ m.value(0, 2)*m.value(2, 1)*m.value(1, 0)
		+ m.value(0, 1)*m.value(1, 2)*m.value(2, 0)
		- m.value(2, 0)*m.value(1, 1)*m.value(0, 2)
		- m.value(0, 0)*m.value(1, 2)*m.value(2, 1)
		- m.value(0, 1)*m.value(1, 0)*m.value(2, 2);
	return v;
}
