...
static void _rg_get_line(real_t p0x, real_t p0y, real_t p1x, real_t p1y, real_t w, real_t ratio, PoolRealArray &points) {
	...
	if (dir_norm != 0) {
		dirx /= dir_norm; diry /= dir_norm;
	}

	const real_t norx = -diry; const real_t nory = dirx;
	const real_t sdx = w; const real_t sdy = ratio * w;
	const real_t dNx = sdx * norx; const real_t dNy = sdy * nory;

	const real_t ax = p0x - dNx; const real_t ay = p0y - dNy;
	const real_t bx = p1x - dNx; const real_t by = p1y - dNy;
	const real_t cx = p1x + dNx; const real_t cy = p1y + dNy;
	const real_t dx = p0x + dNx; const real_t dy = p0y + dNy;

	points.push_back(ax); points.push_back(ay); points.push_back((points.size()/3)%6);
	points.push_back(bx); points.push_back(by); points.push_back((points.size()/3)%6);
	points.push_back(cx); points.push_back(cy); points.push_back((points.size()/3)%6);
	points.push_back(ax); points.push_back(ay); points.push_back((points.size()/3)%6);
	points.push_back(cx); points.push_back(cy); points.push_back((points.size()/3)%6);
	points.push_back(dx); points.push_back(dy); points.push_back((points.size()/3)%6);
}
...
static void _rg_get_rectangle(const real_t p0x, const real_t p0y, const real_t p1x, const real_t p1y, const real_t w, PoolRealArray &points) {
	...
	const real_t ax = p0x - wx; const real_t ay = p0y;
	const real_t bx = p0x + wx; const real_t by = p0y;
	const real_t cx = p1x + wx; const real_t cy = p1y;
	const real_t dx = p1x - wx; const real_t dy = p1y;
...
static void _rg_get_point(real_t p0x, real_t p0y, real_t radius, real_t ratio, PoolRealArray &points) {
	const real_t wx = radius;   const real_t wy = radius * ratio;
	const real_t ax = p0x - wx; const real_t ay = p0y - wy;
	const real_t bx = p0x + wx; const real_t by = p0y - wy;
	const real_t cx = p0x + wx; const real_t cy = p0y + wy;
	const real_t dx = p0x - wx; const real_t dy = p0y + wy;
...
static void _rg_generate_hist(const _rg_graph *graph, const PoolRealArray &ys, _rg_curve *curve) {
	...
	PoolIntArray bin_counts; bin_counts.resize(curve->param1);
...
static void _rg_internal_setup() {
	...
	_shaders[0]->set_code(_rg_fstr);
	_shaders[1] = Ref<Shader>(memnew(Shader));
	_shaders[1]->set_code(_rg_lfstr);
	_shaders[2] = Ref<Shader>(memnew(Shader));
	_shaders[2]->set_code(_rg_pfstr);
	_rg_is_init = true;
}
...
unsigned *rg_palette(const char *name, int num_colors){
	...
	if (strcmp(name, "warm") == 0) {
		switch (num_colors) {
			...
			case 10:
			case 11:
			case 12: return warm[num_colors];
			default: return 0;
		}
		...
		switch (num_colors) {
			...
			case 10:
			case 11:
			case 12: return cool[num_colors];
			default: return 0;
		}
		...
		switch (num_colors) {
			...
			case 10:
			case 11:
			case 12: return neon[num_colors];
			default: return 0;
		}
	}
	return 0;
