...
Font::Font(const void* raw_data, size_t length) {
	// sanity check
	if (length == 0) ERR_THROW(FileLengthError());
...
void Font::VerifyTableCheckSums() const {
	for (auto i = table_map.begin(); i != table_map.end(); ++i) {
		if (VerifyTableCheckSum(i->second) == false) ERR_THROW(ChecksumException(i->second.tagstr));
		}
	}
...
bool Font::ReadGlyphFlags(FItr& itr, matrix3x2t& matrix, uint16_t& glyph_index) const {
	...
	if (abs(abs(a) - abs(c)) <= k) m <<= 1; // * 2.0
	if (abs(abs(c) - abs(d)) <= k) n <<= 1; // * 2.0
...
GlyphMetrics Font::GetGlyphMetrics(uint16_t glyph_index) const {
	...
	// sanity check
	if (version != 0x00010000) ERR_THROW_V(VersionException("Invalid 'hhea' table version."), GlyphMetrics());
	if (metric_data_format != 0) ERR_THROW_V(VersionException("Invalid 'hhea' metricDataFormat."), GlyphMetrics());
...
VGlyphMetrics Font::GetVGlyphMetrics(uint16_t glyph_index) const {
	...
	// sanity check
	if (version != 0x00010000) ERR_THROW_V(VersionException("Invalid 'vhea' table version."), VGlyphMetrics());
	if (metric_data_format != 0) ERR_THROW_V(VersionException("Invalid 'vhea' metricDataFormat."), VGlyphMetrics());
