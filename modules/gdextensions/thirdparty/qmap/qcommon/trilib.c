		...
		*(int *)&start = BigLong(*(int *)&start);
		if (*(int *)&start != exitpattern)
		{
			if (start == FLOAT_START) {
				...
				/* indent(); fprintf(stdout,"OBJECT START: %s\n",name); */
				fread( &count, sizeof(int), 1, input);
				count = BigLong(count);
				++iLevel;
				if (count != 0) {
					/* indent(); fprintf(stdout,"NUMBER OF TRIANGLES: %d\n",count); */
					i = -1;
					do {
						++i;
					...
					/* indent(); fprintf(stdout,"  Object texture name: '%s'\n",tex); */
				}
				...
				/* indent(); fprintf(stdout,"OBJECT END: %s\n",name); */
				continue;
			}
		}
