...
void GdHistoryPlot::draw(const Rect2 &p_frame) {
	...
	switch (range_mode) {
		case RANGE_MANUAL: plot_low = manual_lowest; plot_high = manual_highest; break;
		case RANGE_LOWER_FIXED: plot_low = manual_lowest; plot_high = highest; break;
		case RANGE_AUTOMATIC: plot_low = lowest; plot_high = highest; break;
	}
	...
	if (draw_header) {
		...
		if (draw_numerical_info) {
			...
			String text = have_data ?
				vformat("%s \x12%s \x14%s", humanize_string ? String::humanize_size(cval) : String::num(cval, precision), humanize_string ? String::humanize_size(plot_high) : String::num(plot_high, precision), String::num(plot_low, precision))
...
real_t GdHistoryPlot::get_lower_range() const {
	switch (range_mode) {
		case RANGE_MANUAL: return manual_lowest;
		case RANGE_LOWER_FIXED: return manual_lowest;
...
real_t GdHistoryPlot::get_higer_range() const {
	switch (range_mode) {
		case RANGE_MANUAL: return manual_highest;
...
void GdHistoryPlot::set_draw_skip_val(int p_skip) {
	draw_skip = p_skip; if (draw_skip < 1) {
		draw_skip = 1;
	}
}
...
bool GdHistoryPlot::_prep_vec_glyph(PoolVector2Array &p_data, uint8_t p_idx, const Point2 &p_pos, const Size2 &p_scale) {
	...
	for (int j = start; j < start + num; j++) {
		...
		if (vecfont_ops[j]) {
			if (orig == xy) {
				// point
				p_data.push_back(xy + Vector2(-p_scale.x, 0)); p_data.push_back(xy + Vector2(0, p_scale.y));
				p_data.push_back(xy + Vector2(0, p_scale.y)); p_data.push_back(xy + Vector2(p_scale.x, 0));
				p_data.push_back(xy + Vector2(p_scale.x, 0)); p_data.push_back(xy + Vector2(0, -p_scale.y));
				p_data.push_back(xy + Vector2(0, -p_scale.y)); p_data.push_back(xy + Vector2(-p_scale.x, 0));
			} else {
