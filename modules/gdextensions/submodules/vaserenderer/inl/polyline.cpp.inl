...
void push_quad_(short line, vertex_array_holder& core, const Point& P0, const Point& P1, const Point& P2, const Point& P3, const Color& c0, const Color& c1, const Color& c2, const Color& c3) {
...
void push_quadf_(short line, vertex_array_holder& core, const Point& P0, const Point& P1, const Point& P2, const Point& P3, const Color& c0, const Color& c1, const Color& c2, const Color& c3, bool trans0, bool trans1, bool trans2, bool trans3) {
	if (P0.is_zero()) DEBUG("pushed P0 (0,0) at %d\n",line);
	if (P1.is_zero()) DEBUG("pushed P1 (0,0) at %d\n",line);
	if (P2.is_zero()) DEBUG("pushed P2 (0,0) at %d\n",line);
	if (P3.is_zero()) DEBUG("pushed P3 (0,0) at %d\n",line);
...
// perform knife cut on all triangles (Mesh::PRIMITIVE_TRIANGLES) in core
...
void vah_N_knife_cut(vertex_array_holder &in, vertex_array_holder &out, const Point *kn0, const Point *kn1, const Point *kn2, const Color *kC0, const Color *kC1, int N) {
			...
			for (int p=0; p<cur_count; p++) { // each triangle to be cut
				...
				if (ST[p].T1c > 0) {
					if (kn_colored) {
						...
						triangle_knife_cut(kn0[k], kn1[k], kn2[k], 0, 0, ST[p]);
					}
				}
				// push retaining part
				if (ST[p].T1c > 0) {
...
const real_t cri_core_adapt = 0.0001;
void anchor_late(const Vector2* P, const Color* C, st_polyline* SL, vertex_array_holder& tris, Point cap1, Point cap2) {
	...
	// outer fade, whether degen or normal
	{
		...
		switch( SL[1].djoint) { //line fade joint
			...
			case PLJ_round: {
				vertex_array_holder strip;
				strip.set_draw_mode(Mesh::PRIMITIVE_TRIANGLES);
				Color C2 = C[1]; C2.a = 0;
...
void anchor_cap( const Vector2* P, const Color* C, st_polyline* SL, vertex_array_holder& tris, Point cap1, Point cap2) {
	...
	for (int i = 0, k = 0; k <= 1; i = 2, k++) {
		...
		if (cur_cap.non_zero()) {
			cap.set_draw_mode(Mesh::PRIMITIVE_TRIANGLES);
			bool perform_cut = (SL[1].degenR && SL[1].R_full_degen) && ((k==0 && !SL[1].pre_full) || (k==1 &&  SL[1].pre_full));
			...
			if (SL[i].djoint == PLC_round) {
				//round caps
				{
					...
					vectors_to_arc( strip, O, C[i], C[i], SL[i].T+bR, -SL[i].T+bR, dangle,
					SL[i].t, 0.0, false, &app_P);
					...
					strip.push( a1,C[i]);
					strip.push( a2,C2);
					vectors_to_arc( strip, O, C[i], C2, SL[i].T+bR, -SL[i].T+bR, dangle, SL[i].t, SL[i].t+SL[i].r, false, 0);
					strip.push( b1,C[i]);
					strip.push( b2,C2);
					cap.push(strip);
				}
...
void segment_late(const Vector2* P, const Color* C, st_polyline* SL, vertex_array_holder& tris, Point cap1, Point cap2) {
	tris.set_draw_mode(Mesh::PRIMITIVE_TRIANGLES);
	...
	for (int j = 0; j < 2; j++) {
		vertex_array_holder cap;
		cap.set_draw_mode(Mesh::PRIMITIVE_TRIANGLE_STRIP);
...
int anchor(st_anchor &SA, const polyline_opt* options, bool cap_first, bool cap_last) {
	...
	SA.vah.set_draw_mode(Mesh::PRIMITIVE_TRIANGLES);
	SA.cap_start = Point();
	SA.cap_end = Point();
	...
	}
	{
		...
		// 2nd to 2nd last point
		{
			...
			const bool zero_degree = Point::negligible(cos_tho - 1);
			const bool d180_degree = cos_tho < -1+0.0001;
			char result3 = 1;
	...
	const polyline_inopt *inopt)
{
	...
	vcore.set_draw_mode(Mesh::PRIMITIVE_TRIANGLE_STRIP);
	vfadeo.set_draw_mode(Mesh::PRIMITIVE_TRIANGLE_STRIP);
	vfadei.set_draw_mode(Mesh::PRIMITIVE_TRIANGLE_STRIP);
	...
	const polyline_inopt *in_options) // internal options
{
	...
	for (int i = 1; i < length-1; i++) {
		...
		const real_t cos_a = Math::cos(15 * Math_PI / 180);
		const real_t cos_b = Math::cos(10 * Math_PI / 180);
		const real_t cos_c = Math::cos(25 * Math_PI / 180);
